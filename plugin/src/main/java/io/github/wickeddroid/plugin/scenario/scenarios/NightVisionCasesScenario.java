...
public class NightVisionCasesScenario extends ListenerScenario {
  ...
  public void onEffectRemove(PlayerItemConsumeEvent event) {
    ...
    if(event.getItem().getType() == Material.MILK_BUCKET) {
      Bukkit.getScheduler().runTaskLater(plugin, ()->event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.NIGHT_VISION, PotionEffect.INFINITE_DURATION, 0, false, false, false)), 2L);
    }
  }
}
