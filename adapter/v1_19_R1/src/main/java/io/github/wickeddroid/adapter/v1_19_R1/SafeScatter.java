package io.github.wickeddroid.adapter.v1_19_R1;
...
public class SafeScatter implements ScatterTask {
    @Override
    public @NotNull CompletableFuture<List<Location>> scatterTask(String worldName, int maxX, int maxZ, int count, boolean preventLiquid, boolean aboveSeaLevel, List<Biome> bannedBiomes, EnvironmentAdapter adapter, Consumer<Integer> percentage) throws Exception {
        ...
        for(int i = 0; locations.size() < count; i++) {
            ...
            Consumer<?> consumer = (Consumer<Object>) o -> {
                ...
                locations.add(loc);
                percentage.accept(locations.size());
            };
